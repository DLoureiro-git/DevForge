...
export class CodeValidator {
     ...
     */
    async validateFile(filePath, fileContent) {
    ...
    async validateBatch(files) {
        const results = await Promise.all(files.map(f => this.validateFile(f.path, f.content)));
...
- WARNING: viola boa prática mas não quebra nada`;
     ...
     */
    buildValidationPrompt(filePath, content) {
...
Analisa e reporta TODAS as violações encontradas em formato JSON.`;
     ...
     */
    parseValidationResponse(response, filePath) {
        try {
            ...
            return {
                passed: parsed.violations.length === 0 ||
                    parsed.violations.every((v) => v.severity !== 'ERROR'),
    ...
    buildBatchSummary(results) {
        ...
        const totalErrors = results.reduce((sum, r) => sum + r.violations.filter(v => v.severity === 'ERROR').length, 0);
