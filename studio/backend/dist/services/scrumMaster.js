  ...
  "recommendations": ["string"]
    ...
    async planSprint(projectId, number) {
        ...
        const backlogContext = project.features
            .map((f) => `- ${f.title} (Priority: ${f.priority}, Story Points: ${f.storyPoints || 'unknown'})\n  ${f.description}`)
            .join('\n\n');
        const response = await this.client.messages.create({
            model: 'claude-sonnet-4-20250514',
            max_tokens: 2048,
...
Be constructive and specific. Return JSON format:
{
  ...
  "nextSprintRecommendations": ["string"]
     ...
     */
    async checkWIPLimit(projectId, targetStatus) {
        const WIP_LIMITS = {
        ...
        const limit = WIP_LIMITS[targetStatus];
        if (!limit || limit === 999) {
     ...
     */
    async estimateStoryPoints(feature) {
        const response = await this.client.messages.create({
            model: 'claude-sonnet-4-20250514',
            max_tokens: 256,
            temperature: 0.3,
