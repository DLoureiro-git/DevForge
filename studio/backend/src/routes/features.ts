...
router.get('/:id/features', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    ...
    const features = await prisma.feature.findMany({
      where: {
        ...
        ...(sprint && { sprintId: sprint as string }),
        ...(assignedTo && { assignedAgentId: assignedTo as string }),
      },
      include: {
        sprint: {
          select: {
...
router.post('/:id/features', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const projectId = getParamId(req.params.id);
...
router.put('/:id/features/:fid', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const projectId = getParamId(req.params.id);
    ...
    if (status === 'IN_PROGRESS' && existingFeature.status !== 'IN_PROGRESS') {
      ...
      if (settings?.anthropicKey) {
        ...
        if (!canMove) {
          throw new AppError(
            'WIP limit reached for IN_PROGRESS. Complete some features first.',
            400
          );
        }
      }
...
router.delete('/:id/features/:fid', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const projectId = getParamId(req.params.id);
    const featureId = getParamId(req.params.fid);
    await ensureProjectOwner(projectId, req.user!.id);
...
router.post('/:id/features/:fid/start-build', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const projectId = getParamId(req.params.id);
    const featureId = getParamId(req.params.fid);
    const project = await ensureProjectOwner(projectId, req.user!.id);
...
router.post('/:id/features/:fid/comment', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const projectId = getParamId(req.params.id);
    const featureId = getParamId(req.params.fid);
    await ensureProjectOwner(projectId, req.user!.id);
