...
router.get('/:id/sprints', requireAuth, async (req: AuthRequest, res, next) => {
  try {
...
router.post('/:id/sprints', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const projectId = getParamId(req.params.id);
    await ensureProjectOwner(projectId, req.user!.id);
...
router.put('/:id/sprints/:sid', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const projectId = getParamId(req.params.id);
...
router.post('/:id/sprints/:sid/start', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const projectId = getParamId(req.params.id);
    const sprintId = getParamId(req.params.sid);
    await ensureProjectOwner(projectId, req.user!.id);
...
router.post('/:id/sprints/:sid/end', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const projectId = getParamId(req.params.id);
    const sprintId = getParamId(req.params.sid);
    await ensureProjectOwner(projectId, req.user!.id);
    ...
    const completedStoryPoints = completedFeatures.reduce(
      (sum, f) => sum + (f.storyPoints || 0),
      0
    );
