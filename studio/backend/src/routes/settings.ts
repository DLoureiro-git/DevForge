...
router.put('/', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    ...
    if (ollamaModelDev !== undefined) data.ollamaModelDev = ollamaModelDev;
    if (ollamaModelFix !== undefined) data.ollamaModelFix = ollamaModelFix;
    if (outputDirectory !== undefined) data.outputDirectory = outputDirectory;
    if (notifyEmail !== undefined) data.notifyEmail = notifyEmail;
    if (notifyDesktop !== undefined) data.notifyDesktop = notifyDesktop;
    if (deployTarget !== undefined) data.deployTarget = deployTarget;
