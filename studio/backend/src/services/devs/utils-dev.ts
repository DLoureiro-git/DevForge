...
export interface UtilsDevRequest {
  architecture: string
  filePath: string
...
export interface UtilsDevResponse {
  success: boolean
  code?: string
  filePath: string
  duration: number
...
export class UtilsDev {
  ...
  async generateCode(request: UtilsDevRequest): Promise<UtilsDevResponse> {
