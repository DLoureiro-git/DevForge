...
interface Question {
  ...
  quickReplies?: string[] // Botões de resposta rápida
}

// Árvore de decisão — perguntas do PM Agent
const QUESTION_TREE: Question[] = [
...
export class PMAgent {
   ...
   */
  private reconstructState(messages: Message[]) {
   ...
   */
  private getNextRelevantQuestion(): Question | null {
...
MESSAGE: [tua mensagem]`
   ...
   */
  private formatQuestionWithEncouragement(acknowledgment: string, question: Question): string {
   ...
   */
  private isReadyToBuild(): boolean {
...
**Posso avançar?** (responde "sim" para começar ou corrige algo se necessário)`
  ...
  private estimateTime(): number {
    ...
    if (this.answers.multiUser === 'Sim') minutes += 8
    if (this.answers.monetization && this.answers.monetization !== 'Grátis') minutes += 10
    if (this.answers.reporting === 'Sim, é importante') minutes += 7
    if (this.answers.integrations) minutes += 5
   ...
   */
  private getFeaturesSummary(): string {
   ...
   * Resumo de design
   */
  private getDesignSummary(): string {
   ...
   */
  private buildStartMessage(prd: any): string {
