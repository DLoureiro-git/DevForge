...
describe('CodeValidator', () => {
  let mockArchitecture: ArchitectureOutput
...
export async function GET() {
  ...
  return NextResponse.json(deals)
      ...
      expect(result.passed).toBe(false)
      expect(result.violations.some(v =>
        v.rule.includes('Regra #2') &&
        v.description.toLowerCase().includes('businessid')
      )).toBe(true)
    })
...
export async function hashPassword(password: string) {
  return bcrypt.hash(password, 10)  // Deveria ser 12
      ...
      expect(result.passed).toBe(false)
      expect(result.violations.some(v =>
        v.rule.includes('Regra #3') &&
        v.description.toLowerCase().includes('12 rounds')
      )).toBe(true)
    })
...
export function processData(data: any) {
  return data.map((item: any) => item.value)
      ...
      expect(result.passed).toBe(false)
      expect(result.violations.some(v =>
        v.rule.includes('Regra #5') &&
        v.description.toLowerCase().includes('any')
      )).toBe(true)
    })
...
export function getUser() {
  ...
  return prisma.user.findUnique({ where: { id: 123 } })
      ...
      expect(result.violations.some(v =>
        v.rule.includes('Regra #5') &&
        v.description.toLowerCase().includes('@ts-ignore')
      )).toBe(true)
    })
  })
  ...
  describe('Edge Cases', () => {
    it('deve handle resposta inválida do Ollama gracefully', async () => {
      ...
      const result = await validator.validateFile(
        'invalid.ts',
        'invalid code that crashes ollama'
      )
    ...
    it('deve classificar severidades correctamente', async () => {
      ...
      const result = {
        passed: false,
        violations: [
          { rule: 'R1', file: 'a.ts', description: 'X', severity: 'ERROR' as const },
          { rule: 'R2', file: 'b.ts', description: 'Y', severity: 'ERROR' as const },
          { rule: 'R3', file: 'c.ts', description: 'Z', severity: 'WARNING' as const }
        ],
