...
import type { QAExecutionResult } from './qa-executor';
...
export async function pmContinuousValidation(params: {
  ...
  prd: string;
  minScore?: number;
}): Promise<{ passed: boolean; result: QAExecutionResult }> {
  ...
  const passed =
    result.score.percentage >= minScore &&
    result.score.blockers.length === 0;
...
export async function pmFullWorkflow(params: {
  ...
  if (fullResult.allBugs.length > 0 && fullResult.score.percentage < 90) {
    ...
    if (fixResult.autoFixResults) {
      const fixed = fixResult.autoFixResults.filter((r: any) => r.fixed).length;
 ...
 */
export function pmNotifyQAResults(
