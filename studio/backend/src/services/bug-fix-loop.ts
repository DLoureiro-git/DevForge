...
import type { Bug, QACheck, ValidationResult } from './qa-engine';
...
export interface FixLoopResult {
  ...
  finalStatus: 'fixed' | 'abandoned' | 'escalated';
}
...
const CHECK_TO_FILES_MAP: Record<string, string[]> = {
...
export async function runBugFixLoop(
  ...
  for (let iteration = 1; iteration <= fullConfig.maxIterations; iteration++) {
    ...
    const bugStillExists = validationResults.some(r =>
      r.bugs.some(b => b.checkId === currentBug.checkId && b.severity === currentBug.severity)
    );
  ...
  const finalStatus: 'fixed' | 'abandoned' | 'escalated' =
    fixed ? 'fixed' :
...
function selectModel(iteration: number): 'ollama' | 'haiku' | 'sonnet' {
  if (iteration <= 3) return 'ollama';
  if (iteration <= 6) return 'haiku';
  return 'sonnet';
}
...
async function generateFixOllama(prompt: string, config: FixLoopConfig): Promise<FixResult | null> {
  try {
...
async function generateFixClaude(
  prompt: string,
  model: 'haiku' | 'sonnet',
  config: FixLoopConfig
): Promise<FixResult | null> {
...
async function applyFix(fix: FixResult, projectPath: string): Promise<string[]> {
...
function revertChanges(files: string[], projectPath: string): void {
  try {
...
function getRelevantChecks(filesChanged: string[], allChecks: QACheck[]): QACheck[] {
  const relevantCheckIds = new Set<string>();
...
async function rerunChecks(checks: QACheck[], config: FixLoopConfig): Promise<ValidationResult[]> {
...
async function loadValidators(): Promise<Record<string, Function>> {
...
function buildFixPrompt(bug: Bug, projectPath: string): string {
...
export async function runBatchBugFix(
  bugs: Bug[],
  checks: QACheck[],
  config: FixLoopConfig
): Promise<FixLoopResult[]> {
  const results: FixLoopResult[] = [];
