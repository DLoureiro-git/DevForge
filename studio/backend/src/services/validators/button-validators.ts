...
export async function validateAllButtons(
  browser: Browser,
  projectPath: string,
  deployUrl?: string
  ...
  try {
    ...
    const brokenButtons: { text: string; id: string; class: string }[] = [];
    ...
    for (let i = 0; i < Math.min(buttons.length, 5); i++) {
      ...
      try {
        ...
        if (urlBefore === urlAfter && !hasModal) {
          ...
          const isSubmit = await button.evaluate(el => (el as HTMLButtonElement).type === 'submit');
...
export async function validateLoadingStates(
  ...
  try {
    ...
    const buttonsWithoutLoading: string[] = [];
...
export async function validateDisabledStates(
  browser: Browser,
  projectPath: string,
  deployUrl?: string
): Promise<ValidationResult> {
  ...
  try {
    ...
    const clickableDisabled: string[] = [];
