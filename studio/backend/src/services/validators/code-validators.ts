...
export async function validateImports(
  ...
  try {
    ...
    const brokenImports: { file: string; imports: string[] }[] = [];
...
export async function validateNoHardcodedSecrets(
  browser: Browser,
  projectPath: string
): Promise<ValidationResult> {
  const startTime = Date.now();
  ...
  try {
    ...
    const secretsFound: { file: string; secrets: string[] }[] = [];
...
export async function validateNoConsoleLogs(
  browser: Browser,
  projectPath: string
): Promise<ValidationResult> {
  const startTime = Date.now();
